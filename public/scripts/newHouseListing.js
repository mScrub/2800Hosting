...
function loadEventsToMainDiv() {
    $('#ownPosts').empty()
    $.ajax({
        url:' https://2800hostlocal.azurewebsites.net/ownHousePost/read',
...
function deleteEvent() {
    ...
    console.log(postId)
    $.ajax({
        url: ` https://2800hostlocal.azurewebsites.net/housingPost/delete/${postId}`,
...
function submitForm() {
    ...
    if (title === "" || description === "" || price === "" || city === "" || province === "") { // if any of the fields are empty, alert user
        ...
        console.log(title, description, price, time)
        $.ajax({
            url: " https://2800hostlocal.azurewebsites.net/newHousePost/create",
