...
function loadEventsToMainDiv() {
    $('#ownPosts').empty()
    $.ajax({
        url:'https://intense-cove-26415.herokuapp.com//ownHousePost/read',
...
function deleteEvent() {
    ...
    console.log(postId)
    $.ajax({
        url: `https://intense-cove-26415.herokuapp.com//housingPost/delete/${postId}`,
...
function submitForm() {
    ...
    if (title === "" || description === "" || price === "" || city === "" || province === "") { // if any of the fields are empty, alert user
        ...
        console.log(title, description, price, time)
        $.ajax({
            url: "https://intense-cove-26415.herokuapp.com//newHousePost/create",
